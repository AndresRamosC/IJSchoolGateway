...
import com.ijrobotics.ijschoolmanagergateway.config.KafkaProperties;
...
class IjSchoolManagerGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        IjSchoolManagerGatewayKafkaResource kafkaResource = new IjSchoolManagerGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ij-school-manager-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/ij-school-manager-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
